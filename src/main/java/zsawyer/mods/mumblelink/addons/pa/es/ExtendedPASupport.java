...
public class ExtendedPASupport implements Activateable, IdentityManipulator {
    ...
    private void appendToIdentity(JSONObject identity, Minecraft game)
        ...
        spawnCoordinates.put(game.level.getLevelData().getXSpawn());
        spawnCoordinates.put(game.level.getLevelData().getYSpawn());
        spawnCoordinates.put(game.level.getLevelData().getZSpawn());
        // append coordinates
        identity.put(IdentityKey.WORLD_SPAWN, spawnCoordinates);

        // append the dimension
        identity.put(IdentityKey.DIMENSION, game.player.clientLevel.dimensionType());
    }
